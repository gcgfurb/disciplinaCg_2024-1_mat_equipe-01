...
namespace gcgcg
{
  public class Mundo : GameWindow
  {
    ...
    if (estadoTeclado.IsKeyPressed(Keys.D3) && objetoSelecionado != null)   //FIXME: problema depois de usa rotação pto qquer, não usa o novo centro da BBOX
      ...
      {
        Ponto4D ptoClick = new Ponto4D(((MousePosition[0] - (ClientSize[0]/2))/4) * 0.01, -((MousePosition[1] - (ClientSize[1]/2))/4) * 0.01);
        int count;
        Objeto obj;
        ...
        for(int i = 0; i < objetos.Count; i++){
          ...
          if(obj.Bbox().Dentro(ptoClick)) {
            for(int j = 0; j < obj.PontosListaTamanho; j++) {
              if(j < obj.PontosListaTamanho - 1){
                if(Matematica.ScanLine(ptoClick, obj.PontosId(j), obj.PontosId(j+1))) {
                  count++;
                }
              } else {
                if(Matematica.ScanLine(ptoClick, obj.PontosId(j), obj.PontosId(0))) {
                  count++;
                }
              }
            }
