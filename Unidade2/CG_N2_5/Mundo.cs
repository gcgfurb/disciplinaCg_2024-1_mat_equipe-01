    ...
    public class Mundo : GameWindow
    {
      ...
      -0.5f,  0.0f,  0.0f, /* X- */      0.5f,  0.0f,  0.0f, /* X+ */
       ...
       0.0f,  0.0f, -0.5f, /* Z- */      0.0f,  0.0f,  0.5f, /* Z+ */
        ...
        private Circulo circuloIn;
        private Circulo circuloOut;

        private Ponto4D centro;
        private Ponto centroFig;

        private BBox boundingBox;

        List<Ponto4D> listaBBox = new List<Ponto4D>();

        private Retangulo BBoxFig;
        ...
        protected override void OnLoad()
        {
            ...
            circuloOut = new Circulo(mundo, ref rotuloAtual, 0.2, 0.2);
            circuloOut.shaderObjeto = new Shader("Shaders/shader.vert", "Shaders/shaderAmarela.frag");

            circuloIn = new Circulo(mundo, ref rotuloAtual, 0.1, 0.2);

            centro = new Ponto4D(0.2, 0.2);
            centroFig = new Ponto(mundo, ref rotuloAtual, centro);
            centroFig.PrimitivaTamanho = 5;
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (input.IsKeyPressed(Keys.C)) {
                                if(!boundingBox.Dentro(centro)) {
                                    BBoxFig.PrimitivaTipo = PrimitiveType.Points;

                                    if(Matematica.distanciaQuadrado(new Ponto4D(centro.X, centro.Y+0.01), new Ponto4D(0.2, 0.2)) < 0.04) {
                                        moveJoystick(0.0, 0.01);
                                    }
                                } else {
                                    BBoxFig.PrimitivaTipo = PrimitiveType.LineLoop;
                                    moveJoystick(0.0, 0.01);
                                ...
                                if(!boundingBox.Dentro(centro)) {
                                    ...
                                    if(Matematica.distanciaQuadrado(new Ponto4D(centro.X-0.01, centro.Y), new Ponto4D(0.2, 0.2)) < 0.04) {
                                        moveJoystick(-0.01, 0.0);
                                    }
                                } else {
                                    BBoxFig.PrimitivaTipo = PrimitiveType.LineLoop;
                                    moveJoystick(-0.01, 0.0);
        ...
        private void moveJoystick(double qtdX, double qtdY) {
            circuloIn.deslocaCirculo(qtdX, qtdY);
            objetoSelecionado = circuloIn;

            centro.X += qtdX;
            centro.Y += qtdY;

            centroFig.PontosAlterar(new Ponto4D(centro.X, centro.Y), 0);
        }
